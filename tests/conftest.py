...
def fixture_users(vault_instance, postgres_instance):
    ...
    users = [
        ...
        # - RATE_LIMIT: restrictions on requests have not yet been applied
        {
            ...
            'requests_history': [
                (
                    'testUser5', 'testMessage1', 'testChat5', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc)
                )
            ]
        },
        #
        ...
        # - RATE_LIMIT: restrictions on requests have not yet been applied
        {
            ...
            'requests_history': [
                (
                    'testUser6', 'testMessage1', 'testChat6', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=3)
                ),
                (
                    'testUser6', 'testMessage2', 'testChat6', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=4)
                ),
                (
                    'testUser6', 'testMessage3', 'testChat6', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=5)
                ),
                (
                    'testUser6', 'testMessage4', 'testChat6', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=6), datetime.now(timezone.utc) + timedelta(hours=24)
                ),
            ]
        },
        #
        ...
        # - RATE_LIMIT: restrictions on requests have not yet been applied
        {
            ...
            'requests_history': [
                (
                    'testUser7', 'testMessage1', 'testChat7', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=1)
                ),
                (
                    'testUser7', 'testMessage2', 'testChat7', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}',
                    datetime.now(timezone.utc) - timedelta(hours=1),
                    datetime.now(timezone.utc) + timedelta(hours=1, minutes=15)
                ),
                (
                    'testUser7', 'testMessage3', 'testChat7', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=3)),
                (
                    'testUser7', 'testMessage4', 'testChat7', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=4), datetime.now(timezone.utc) + timedelta(hours=24)
                ),
            ]
        },
        # Test user8
        # - RATE_LIMIT: limited requests
        {
            ...
            'status': 'allowed',
            'roles': ['admin_role'],
            'requests': {'requests_per_day': 10, 'requests_per_hour': 3, 'random_shift_minutes': 15},
            'requests_history': []
        },
        # Test user9
        # - RATE_LIMIT: limited requests
        # - RATE_LIMIT: requests limit PER HOUR exist but expired
        {
            'name': 'testUser9',
            'status': 'allowed',
            'roles': ['admin_role'],
            ...
            'requests_history': [
                (
                    'testUser9', 'testMessage1', 'testChat9', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=2)
                ),
                (
                    'testUser9', 'testMessage2', 'testChat9', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=2, minutes=10)
                ),
                (
                    'testUser9', 'testMessage3', 'testChat9', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=2, minutes=15)
                )
            ]
        },
        # Test user10
        # - RATE_LIMIT: limited requests
        # - RATE_LIMIT: requests limit PER DAY exist but expired
        {
            'name': 'testUser10',
            'status': 'allowed',
            'roles': ['admin_role'],
            ...
            'requests_history': [
                (
                    'testUser10', 'testMessage1', 'testChat10', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=25)
                ),
                (
                    'testUser10', 'testMessage2', 'testChat10', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=27)
                ),
                (
                    'testUser10', 'testMessage3', 'testChat10', 'allowed',
                    '{"role_id": "admin_role", "status": "allowed"}', datetime.now(timezone.utc) - timedelta(hours=29)
                )
            ]
        },
        # Test user11
